...
class DeeperDeepSpeech(nn.Module):
    ...
    def __init__(self, vocab_size):
        ...
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=20, kernel_size=3, stride=3)
        self.conv1_bn = nn.BatchNorm2d(20)
        self.conv1_dropout = nn.Dropout(p=0.5)
        self.conv2 = nn.Conv2d(in_channels=20, out_channels=50, kernel_size=3, stride=3)
        self.conv2_bn = nn.BatchNorm2d(50)
        self.conv2_dropout = nn.Dropout(p=0.5)
        self.lstm1 = nn.LSTM(input_size=850, hidden_size=250, bidirectional=True)
        self.lstm1_dropout = nn.Dropout(p=0.5)
        self.lstm2 = nn.LSTM(input_size=500, hidden_size=150, bidirectional=True)
        self.lstm2_dropout = nn.Dropout(p=0.5)
        self.fc1 = nn.Linear(in_features=300, out_features=100)
        self.fc1_bn = nn.BatchNorm1d(11)
