...
def get_characters_indices():
    characters_vocab = [chr(c) for c in range(97, 123)]
    return {c: i for i, c in enumerate(characters_vocab, 1)}
...
def make_dataset_raw_labels(dir):
    spects = []
    dir = os.path.expanduser(dir)
    for target in sorted(os.listdir(dir)):
        d = os.path.join(dir, target)
...
class DatasetLoaderParser(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        path, target = self.spects[index]
        target_seq = self.get_target_seq(target)
        spect = self.loader(path, self.window_size, self.window_stride, self.window_type, self.normalize, self.max_len)
        if self.transform is not None:
            spect = self.transform(spect)
        if self.target_transform is not None:
            target_seq = self.target_transform(target_seq)

        return spect, target_seq
